...
// API 기본 URL - 환경 변수 사용
const API_BASE_URL = import.meta.env.VITE_API_BASE_URL || 'https://doc-update-manager.onrender.com';
 ...
 */
export const checkChatApiReady = async (timeout: number = 5000): Promise<{ ready: boolean; status: string }> => {
  const healthUrl = `${API_BASE_URL}/health`;
  const startTime = performance.now();
  console.log('⚡ 빠른 준비 상태 체크 시작:', healthUrl, `(타임아웃: ${timeout}ms)`);
  ...
  try {
    ...
    if (response.ok) {
      ...
      console.log('⚡ 빠른 준비 상태 응답:', data, `(${responseTime.toFixed(0)}ms)`);
      ...
      console.warn('⚠️ 헬스 체크 HTTP 오류:', response.status, `(${responseTime.toFixed(0)}ms)`);
      return { ready: false, status: 'error' };
    }
  } catch (error) {
    const responseTime = performance.now() - startTime;
    if (error instanceof Error && error.name === 'AbortError') {
      console.error('❌ 빠른 준비 상태 확인 타임아웃:', timeout + 'ms', `(실제: ${responseTime.toFixed(0)}ms)`);
    } else {
      console.error('❌ 빠른 준비 상태 확인 중 오류:', error, `(${responseTime.toFixed(0)}ms)`);
    }
    return { ready: false, status: 'error' };
  }
};
 ...
 */
export const checkChatApiHealth = async (timeout: number = 8000): Promise<boolean> => {
  const healthUrl = `${API_BASE_URL}/health`;
  const startTime = performance.now();
  ...
  try {
    ...
    const responseTime = performance.now() - startTime;
    if (error instanceof Error && error.name === 'AbortError') {
      console.error('❌ API 상태 확인 타임아웃:', timeout + 'ms', `(실제: ${responseTime.toFixed(0)}ms)`);
    } else {
      console.error('❌ API 상태 확인 중 오류:', error, `(${responseTime.toFixed(0)}ms)`);
    }
    return false;
  }
};
...
export const checkChatApiHealthWithRetry = async (
  ...
  console.log(`🔄 헬스 체크 재시도 시작 - 최대 ${maxRetries}회, 기본지연 ${baseDelay}ms, 타임아웃 ${timeout}ms`);
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    ...
    if (isHealthy) {
      console.log(`✅ 헬스 체크 성공 (${attempt}회차, ${attemptTime.toFixed(0)}ms)`);
      return true;
    }
    
    console.warn(`❌ 헬스 체크 실패 (${attempt}회차, ${attemptTime.toFixed(0)}ms)`);
    ...
    if (attempt < maxRetries) {
      ...
      console.log(`⏳ ${actualDelay}ms 후 재시도... (지수 백오프: ${attempt}회차)`);
...
export const testHealthCheckPerformance = async (testCount: number = 5) => {
  ...
  for (let i = 1; i <= testCount; i++) {
    ...
    try {
      ...
      console.log(`✅ 테스트 ${i} 완료: ${duration.toFixed(0)}ms, 성공: ${result.ready}`);
    } catch (error) {
      const endTime = performance.now();
      const duration = endTime - startTime;
      ...
      console.log(`❌ 테스트 ${i} 실패: ${duration.toFixed(0)}ms, 오류: ${error}`);
    }
  ...
  console.log(`  - 성공률: ${successCount}/${testCount} (${((successCount/testCount)*100).toFixed(1)}%)`);
